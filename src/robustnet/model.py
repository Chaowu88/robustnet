...
class Model:
    ...
    def initial_substrates(self, exclude=None):
        ...
        metabolites that participate in only one consuming reaction 
    ...
    def final_products(self, exclude=None):
        ...
        that participate in only one producing reaction and are not consumed
        by any reaction.
    ...
    def evaluate_robustness(
        ...
        fold_change : 2-tuple or dict, optional
            ...
            ``{"A": (1, 10), "B": (1, 50)}`` represents co-overexpression of
            enzyme A and B, whereas ``{"A": (1, 10), "B": (0.5, 1)}`` represents
            simultaneous overexperssion of enzyme A and downregulation of enzyme B.
        ...
        if not hasattr(self, 'init_metab_sets'):
            ...
            return EnsembleResults(*sim_res)
